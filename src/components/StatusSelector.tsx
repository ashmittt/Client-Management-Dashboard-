...
const statusColors: Record<ClientStatus, string> = {
  ...
  completed: "border-status-completed text-status-completed bg-status-completed/10",
};

const statusInactive: Record<ClientStatus, string> = {
  active: "border-border text-text-muted hover:border-status-active/40 hover:text-status-active/60",
  paused: "border-border text-text-muted hover:border-status-paused/40 hover:text-status-paused/60",
  completed: "border-border text-text-muted hover:border-status-completed/40 hover:text-status-completed/60",
};
      ...
      {statuses.map((s) => (
        <button
          ...
          className={`px-3 py-1.5 text-xs font-medium rounded border transition-colors duration-150 disabled:opacity-50 ${
            status === s.value ? statusColors[s.value] : statusInactive[s.value]
          }`}
        >
          {s.label}
        </button>
